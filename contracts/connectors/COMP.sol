...
contract BasicResolver is COMPHelpers {
    event LogClaimedComp(uint256 compAmt);
    ...
    */
    function ClaimComp() external payable {
        ...
        emit LogClaimedComp(amt);
        bytes32 _eventCode = keccak256("LogClaimedComp(uint256)");
        bytes memory _eventParam = abi.encode(amt);
    ...
    */
    function ClaimCompTwo(address[] calldata tokens) external payable {
    ...
    */
    function ClaimCompThree(address[] calldata supplyTokens, address[] calldata borrowTokens) external payable {
