...
contract BasicResolver is CompoundHelpers {
    ...
    */
    function deposit(address token, uint amt) external payable{
        ...
        emit LogDeposit(token, cToken, _amt);
    }
    ...
    */
    function withdraw(address token, uint amt) external payable{
        ...
        emit LogWithdraw(token, cToken, _amt);
    }
    ...
    */
    function borrow(address token, uint amt) external payable {
        ...
        emit LogBorrow(token, cToken, _amt);
    }
    ...
    */
    function payback(address token, uint amt) external payable {
        ...
        emit LogPayback(token, cToken, _amt);
    }
}

contract ExtraResolver is BasicResolver {
    event LogClaimedComp(uint256 compAmt, uint256 setId);
    event LogDepositCToken(address indexed token, address cToken, uint256 tokenAmt, uint256 cTokenAmt);
    event LogWithdrawCToken(address indexed token, address cToken, uint256 tokenAmt, uint256 cTokenAmt);
    event LogLiquidate(
    ...
    */
    function depositCToken(address token, uint amt) external payable{
        uint _amt = amt;
        address cToken = InstaMapping(getMappingAddr()).cTokenMapping(token);
        enterMarket(cToken);
        ...
        emit LogDepositCToken(token, cToken, _amt, _cAmt);
    }
    ...
    */
    function withdrawCToken(address token, uint cTokenAmt) external payable {
        uint _cAmt = cTokenAmt;
        address cToken = InstaMapping(getMappingAddr()).cTokenMapping(token);
        CTokenInterface cTokenContract = CTokenInterface(cToken);
        TokenInterface tokenContract = TokenInterface(token);
        ...
        emit LogWithdrawCToken(token, cToken, withdrawAmt, _cAmt);
    }
